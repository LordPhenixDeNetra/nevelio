  ...
  const TRANSLATIONS = {
    fr: {
      ...
      'nav.ai': 'Fonctionnalités IA',
      'nav.ai.triage': '↳ --ai-triage',
      'nav.ai.remediation': '↳ --ai-remediation',
      'nav.ai.report': '↳ --ai-report',
      'nav.ai.payloads': '↳ --ai-payloads',
      'nav.agent': 'Agent autonome',
      'nav.mcp': 'MCP serve',
      'nav.cicd': 'CI/CD',
      ...
      'toc.8': 'Fonctionnalités IA',
      'toc.8bis': 'Agent autonome',
      'toc.8ter': 'MCP serve',
      ...
      'mode.ai.desc': 'Fonctionnalités IA : <code>--ai-triage</code>, <code>--ai-remediation</code>, <code>--ai-report</code>, <code>--ai-payloads</code> (nécessite un provider configuré)',
      ...
      'ai.desc': 'Nevelio intègre plusieurs fonctionnalités IA activées via des flags de scan. Chacune nécessite un provider configuré (<code>nevelio config init</code>).',
      'ai.prereq': 'Configurer un provider avant d\'utiliser les flags IA : <code>nevelio config init</code> puis <code>nevelio config ai ping</code>',
      'ai.triage.title': 'Triage IA — <code>--ai-triage</code>',
      'ai.triage.desc': 'Classe chaque finding en <strong>vrai positif / faux positif / incertain</strong> avec un score de confiance et une justification. Sortie : <code>ai_triage.json</code>',
      'ai.remediation.title': 'Remédiation IA — <code>--ai-remediation</code>',
      'ai.remediation.desc': 'Génère des <strong>étapes de remédiation détaillées</strong> avec exemples de code. Sortie : <code>ai_remediation.md</code>',
      'ai.report.title': 'Rapport narratif IA — <code>--ai-report</code>',
      'ai.report.desc': 'Génère un <strong>rapport exécutif complet</strong> : résumé, chaîne d\'attaque narrative, priorités. Sortie : <code>ai_narrative_report.md</code>',
      'ai.payloads.title': 'Payloads IA — <code>--ai-payloads</code>',
      'ai.payloads.desc': 'Génère des <strong>payloads d\'attaque contextuels</strong> adaptés au framework et aux endpoints. Sortie : <code>ai_payloads.json</code>',
      'ai.combo.title': 'Combinaison de flags',
      'ai.routing.title': 'Routing par tâche',
      'ai.routing.desc': 'Chaque tâche peut utiliser un provider différent via la config :',
      // Agent
      'agent.title': 'Commande <code>agent</code> — Agent autonome',
      'agent.desc': 'L\'agent autonome pilote une boucle <strong>LLM → outils → analyse</strong> sans intervention humaine. Il découvre les endpoints, les sonde, identifie les vulnérabilités et produit un rapport.',
      'agent.options': 'Options',
      'agent.col.default': 'Défaut',
      'agent.target.desc': 'URL de base de l\'API cible (requis)',
      'agent.iter.desc': 'Nombre maximum de tours LLM',
      'agent.req.desc': 'Nombre maximum de requêtes HTTP',
      'agent.budget.desc': 'Limite de tokens consommés (coût)',
      'agent.dry.desc': 'Planifie sans envoyer de requêtes réelles',
      'agent.outdir.desc': 'Répertoire de sortie du rapport',
      'agent.guardrails': 'Guardrails de sécurité',
      'agent.guardrail.col': 'Guardrail',
      'agent.guardrail.behavior': 'Comportement',
      'agent.g.scope': 'Refuse toute requête hors du domaine cible',
      'agent.g.req': 'Arrêt propre une fois la limite atteinte',
      'agent.g.budget': 'Arrêt si le budget IA est dépassé',
      'agent.g.dry': 'Simule les requêtes sans les envoyer',
      'agent.g.legal': 'Refuse de démarrer sans <code>--accept-legal</code>',
      // MCP
      'mcp.title': 'Commande <code>mcp serve</code> — Serveur MCP',
      'mcp.desc': 'Expose les outils Nevelio via le <strong>Model Context Protocol (MCP)</strong>, permettant à Claude Desktop ou tout agent compatible d\'orchestrer Nevelio depuis une conversation.',
      'mcp.claude-config': 'Configuration Claude Desktop',
      'mcp.claude-config.desc': 'Ajouter dans <code>~/.config/claude/claude_desktop_config.json</code> :',
      'mcp.tools': 'Outils MCP exposés',
      'mcp.col.tool': 'Outil',
      'mcp.tool.list': 'Crawle la cible et retourne les endpoints découverts',
      'mcp.tool.probe': 'Sonde un endpoint (méthode, headers, body configurables)',
      'mcp.tool.report': 'Enregistre une vulnérabilité dans la session',
      'mcp.tool.finish': 'Retourne le rapport JSON complet de la session',
      'mcp.transport': 'Transport : <strong>stdio</strong> — compatible MCP protocol version <code>2024-11-05</code>',
      // Report
      ...
      'flags.ai.desc': 'Fonctionnalités IA : --ai-triage, --ai-remediation, --ai-report, --ai-payloads',
      ...
      'flags.anthropic-key.desc': 'Clé API Anthropic Claude (provider anthropic)',
    ...
    en: {
      ...
      'mode.ai.desc': 'AI features: <code>--ai-triage</code>, <code>--ai-remediation</code>, <code>--ai-report</code>, <code>--ai-payloads</code> (requires a configured provider)',
      ...
      'ai.desc': 'Nevelio integrates several AI features activated via scan flags. Each requires a configured provider (<code>nevelio config init</code>).',
      'ai.prereq': 'Configure a provider before using AI flags: <code>nevelio config init</code> then <code>nevelio config ai ping</code>',
      'ai.triage.title': 'AI Triage — <code>--ai-triage</code>',
      'ai.triage.desc': 'Classifies each finding as <strong>true positive / false positive / uncertain</strong> with a confidence score and justification. Output: <code>ai_triage.json</code>',
      'ai.remediation.title': 'AI Remediation — <code>--ai-remediation</code>',
      'ai.remediation.desc': 'Generates <strong>detailed remediation steps</strong> with code examples. Output: <code>ai_remediation.md</code>',
      'ai.report.title': 'AI Narrative Report — <code>--ai-report</code>',
      'ai.report.desc': 'Generates a <strong>complete executive report</strong>: summary, narrative attack chain, priorities. Output: <code>ai_narrative_report.md</code>',
      'ai.payloads.title': 'AI Payloads — <code>--ai-payloads</code>',
      'ai.payloads.desc': 'Generates <strong>contextual attack payloads</strong> adapted to the framework and endpoints. Output: <code>ai_payloads.json</code>',
      'ai.combo.title': 'Combining flags',
      'ai.routing.title': 'Per-task routing',
      'ai.routing.desc': 'Each task can use a different provider via config:',
      // Agent
      'agent.title': '<code>agent</code> command — Autonomous agent',
      'agent.desc': 'The autonomous agent drives a <strong>LLM → tools → analysis</strong> loop without human intervention. It discovers endpoints, probes them, identifies vulnerabilities and produces a report.',
      'agent.options': 'Options',
      'agent.col.default': 'Default',
      'agent.target.desc': 'Base URL of the target API (required)',
      'agent.iter.desc': 'Maximum number of LLM turns',
      'agent.req.desc': 'Maximum number of HTTP requests',
      'agent.budget.desc': 'Token consumption limit (cost)',
      'agent.dry.desc': 'Plans without sending real requests',
      'agent.outdir.desc': 'Report output directory',
      'agent.guardrails': 'Security guardrails',
      'agent.guardrail.col': 'Guardrail',
      'agent.guardrail.behavior': 'Behaviour',
      'agent.g.scope': 'Refuses any request outside the target domain',
      'agent.g.req': 'Clean stop once the limit is reached',
      'agent.g.budget': 'Stops if the AI budget is exceeded',
      'agent.g.dry': 'Simulates requests without sending them',
      'agent.g.legal': 'Refuses to start without <code>--accept-legal</code>',
      // MCP
      'mcp.title': '<code>mcp serve</code> command — MCP Server',
      'mcp.desc': 'Exposes Nevelio tools via the <strong>Model Context Protocol (MCP)</strong>, allowing Claude Desktop or any compatible agent to orchestrate Nevelio from a conversation.',
      'mcp.claude-config': 'Claude Desktop configuration',
      'mcp.claude-config.desc': 'Add to <code>~/.config/claude/claude_desktop_config.json</code>:',
      'mcp.tools': 'Exposed MCP tools',
      'mcp.col.tool': 'Tool',
      'mcp.tool.list': 'Crawls the target and returns discovered endpoints',
      'mcp.tool.probe': 'Probes an endpoint (method, headers, body configurable)',
      'mcp.tool.report': 'Records a vulnerability in the session',
      'mcp.tool.finish': 'Returns the complete JSON report for the session',
      'mcp.transport': 'Transport: <strong>stdio</strong> — compatible MCP protocol version <code>2024-11-05</code>',
      // Report
      ...
      'flags.ai.desc': 'AI features: --ai-triage, --ai-remediation, --ai-report, --ai-payloads',
      ...
      'flags.col.variable': 'Variable',
      'flags.anthropic-key.desc': 'Anthropic Claude API key (anthropic provider)',
    ...
    es: {
      ...
      'nav.ai': 'Funcionalidades IA',
      'nav.ai.triage': '↳ --ai-triage',
      'nav.ai.remediation': '↳ --ai-remediation',
      'nav.ai.report': '↳ --ai-report',
      'nav.ai.payloads': '↳ --ai-payloads',
      'nav.agent': 'Agente autónomo',
      'nav.mcp': 'MCP serve',
      'nav.cicd': 'CI/CD',
      ...
      'toc.8': 'Funcionalidades IA',
      'toc.8bis': 'Agente autónomo',
      'toc.8ter': 'MCP serve',
      ...
      'mode.ai.desc': 'Funcionalidades IA: <code>--ai-triage</code>, <code>--ai-remediation</code>, <code>--ai-report</code>, <code>--ai-payloads</code> (requiere un provider configurado)',
      ...
      // AI
      'ai.title': 'Funcionalidades IA',
      'ai.desc': 'Nevelio integra varias funcionalidades IA activadas mediante flags de escaneo. Cada una requiere un provider configurado (<code>nevelio config init</code>).',
      'ai.prereq': 'Configurar un provider antes de usar los flags IA: <code>nevelio config init</code> y luego <code>nevelio config ai ping</code>',
      'ai.triage.title': 'Triaje IA — <code>--ai-triage</code>',
      'ai.triage.desc': 'Clasifica cada hallazgo como <strong>verdadero positivo / falso positivo / incierto</strong> con puntuación de confianza y justificación. Salida: <code>ai_triage.json</code>',
      'ai.remediation.title': 'Remediación IA — <code>--ai-remediation</code>',
      'ai.remediation.desc': 'Genera <strong>pasos de remediación detallados</strong> con ejemplos de código. Salida: <code>ai_remediation.md</code>',
      'ai.report.title': 'Informe narrativo IA — <code>--ai-report</code>',
      'ai.report.desc': 'Genera un <strong>informe ejecutivo completo</strong>: resumen, cadena de ataque narrativa, prioridades. Salida: <code>ai_narrative_report.md</code>',
      'ai.payloads.title': 'Payloads IA — <code>--ai-payloads</code>',
      'ai.payloads.desc': 'Genera <strong>payloads de ataque contextuales</strong> adaptados al framework y los endpoints. Salida: <code>ai_payloads.json</code>',
      'ai.combo.title': 'Combinación de flags',
      'ai.routing.title': 'Routing por tarea',
      'ai.routing.desc': 'Cada tarea puede usar un provider diferente via configuración:',
      // Agent
      'agent.title': 'Comando <code>agent</code> — Agente autónomo',
      'agent.desc': 'El agente autónomo gestiona un bucle <strong>LLM → herramientas → análisis</strong> sin intervención humana. Descubre endpoints, los sondea, identifica vulnerabilidades y produce un informe.',
      'agent.options': 'Opciones',
      'agent.col.default': 'Defecto',
      'agent.target.desc': 'URL base de la API objetivo (requerido)',
      'agent.iter.desc': 'Número máximo de turnos LLM',
      'agent.req.desc': 'Número máximo de solicitudes HTTP',
      'agent.budget.desc': 'Límite de tokens consumidos (coste)',
      'agent.dry.desc': 'Planifica sin enviar solicitudes reales',
      'agent.outdir.desc': 'Directorio de salida del informe',
      'agent.guardrails': 'Guardrails de seguridad',
      'agent.guardrail.col': 'Guardrail',
      'agent.guardrail.behavior': 'Comportamiento',
      'agent.g.scope': 'Rechaza cualquier solicitud fuera del dominio objetivo',
      'agent.g.req': 'Parada limpia al alcanzar el límite',
      'agent.g.budget': 'Se detiene si se supera el presupuesto IA',
      'agent.g.dry': 'Simula las solicitudes sin enviarlas',
      'agent.g.legal': 'Se niega a iniciar sin <code>--accept-legal</code>',
      // MCP
      'mcp.title': 'Comando <code>mcp serve</code> — Servidor MCP',
      'mcp.desc': 'Expone las herramientas de Nevelio mediante el <strong>Model Context Protocol (MCP)</strong>, permitiendo que Claude Desktop o cualquier agente compatible orqueste Nevelio desde una conversación.',
      'mcp.claude-config': 'Configuración Claude Desktop',
      'mcp.claude-config.desc': 'Añadir en <code>~/.config/claude/claude_desktop_config.json</code>:',
      'mcp.tools': 'Herramientas MCP expuestas',
      'mcp.col.tool': 'Herramienta',
      'mcp.tool.list': 'Rastrea el objetivo y devuelve los endpoints descubiertos',
      'mcp.tool.probe': 'Sondea un endpoint (método, cabeceras, cuerpo configurables)',
      'mcp.tool.report': 'Registra una vulnerabilidad en la sesión',
      'mcp.tool.finish': 'Devuelve el informe JSON completo de la sesión',
      'mcp.transport': 'Transporte: <strong>stdio</strong> — compatible MCP protocol version <code>2024-11-05</code>',
      // Report
      ...
      'flags.ai.desc': 'Funcionalidades IA: --ai-triage, --ai-remediation, --ai-report, --ai-payloads',
      ...
      'flags.col.variable': 'Variable',
      'flags.anthropic-key.desc': 'Clave API Anthropic Claude (provider anthropic)',
