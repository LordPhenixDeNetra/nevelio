...
pub(super) fn print_banner() {
    ...
    println!(
        "  Tapez {} pour les commandes, {} pour quitter.\n",
        "help".bold(),
        "quit".bold()
    );
}
...
pub(super) fn print_help() {
    ...
    let cmds = [
        (
            "target <url>",
            "Définir la cible (ou afficher la cible actuelle)",
        ),
        (
            "spec <path>",
            "Définir le fichier spec (OpenAPI/Postman/HAR)",
        ),
...
pub(super) fn print_status(ctx: &ShellCtx) {
    println!(
        "  Cible    : {}",
        ctx.target.as_deref().unwrap_or("(non définie)").cyan()
    );
    println!(
        "  Spec     : {}",
        ctx.spec.as_deref().unwrap_or("(aucune)").dimmed()
    );
    println!(
        "  Token    : {}",
    ...
    println!(
        "  Out dir  : {}",
        ctx.out_dir.display().to_string().dimmed()
    );
}
