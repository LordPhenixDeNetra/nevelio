...
pub(super) async fn handle_linear(report: &ScanReport, args: LinearIssueArgs) -> Result<()> {
    ...
    for finding in &to_create {
        ...
        let description = format!(
            ...
            module = finding.module, ep = finding.endpoint, method = finding.method,
            sev = finding.severity, cvss = finding.cvss_score,
            desc = finding.description, rec = finding.recommendation,
            cwe = finding.cwe.as_deref().map(|c| format!("**CWE:** {}\n\n", c)).unwrap_or_default(),
            ver = env!("CARGO_PKG_VERSION"), target = report.target,
        );
        ...
        if let Some(ref pid) = args.project  { variables["projectId"] = serde_json::json!(pid); }
        if let Some(ref lid) = label_id { variables["labelIds"] = serde_json::json!([lid]); }
        ...
        let resp = client.post("https://api.linear.app/graphql")
            ...
            .send().await.context("Erreur appel Linear API")?;

        let body: serde_json::Value = resp.json().await?;
        if body["data"]["issueCreate"]["success"].as_bool().unwrap_or(false) {
            let url = body["data"]["issueCreate"]["issue"]["url"].as_str().unwrap_or("");
    ...
    println!("\n  Bilan : {} issue(s) créé(s), {} ignoré(s) (déjà existants).", created, skipped);
    Ok(())
}
