...
impl AttackModule for OAuth2Module {
    ...
    async fn run(
        ...
        findings.extend(probe_token_referer_leak(client, base, &auth_endpoints).await);
...
async fn probe_token_referer_leak(
    client: &HttpClient,
    base: &str,
    auth_endpoints: &[&Endpoint],
) -> Vec<Finding> {
                    ...
                    f.description =
                        "Le token endpoint redirige avec le token d'accès dans l'URL \
                         ...
                         du serveur et transmis aux tiers via le header Referer."
                            .to_string();
                    f.proof = format!("POST {} → HTTP {} Location: {}", url, status, &loc[..loc.len().min(200)]);
                    f.recommendation =
...
fn location_has_query_token(status: u16, body: &str) -> bool {
