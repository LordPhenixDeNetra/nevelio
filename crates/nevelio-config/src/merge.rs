...
pub fn merge(
    ...
    // Project overrides
    if let Some(p) = &project.profile {
        scan.profile = Some(p.clone());
    }
    if let Some(d) = project.dry_run {
        scan.dry_run = d;
    }
    if let Some(t) = project.timeout {
        scan.timeout_secs = t;
    }
    if let Some(c) = project.concurrency {
        scan.concurrency = c;
    }
    if let Some(l) = &project.lang {
        scan.lang = l.clone();
    }

    // CLI overrides
    if let Some(l) = overrides.lang {
        scan.lang = l;
    }
    if let Some(d) = overrides.dry_run {
        scan.dry_run = d;
    }
