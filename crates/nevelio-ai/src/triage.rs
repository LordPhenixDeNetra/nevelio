...
#[derive(Deserialize)]
struct LlmWrapper { findings: Vec<LlmTriage> }
...
pub async fn classify_findings(
    ...
    let finding_list: Vec<Value> = findings.iter().map(|f| json!({
        "id":             f.id,
        "title":          f.title,
        "severity":       f.severity,
        "module":         f.module,
...
{{"findings": [{{"id": "...", "verdict": "true_positive|false_positive|uncertain", "confidence": 0-100, "reason": "..."}}]}}"#,
    ...
    let results: Vec<TriageResult> = wrapper.findings.into_iter().map(|t| {
        let verdict = match t.verdict.as_str() {
            "true_positive"  => Verdict::TruePositive,
            "false_positive" => Verdict::FalsePositive,
            _                => Verdict::Uncertain,
        };
        TriageResult { id: t.id, verdict, confidence: t.confidence, reason: t.reason }
    }).collect();
