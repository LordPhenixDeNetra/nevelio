    ...
    fn build_converse_body(&self, messages: &[Message], opts: &CompletionOpts) -> Value {
        ...
        let chat: Vec<Value> = messages
            ...
            .filter_map(|m| match m.role {
                ...
                Role::User => {
                    Some(json!({ "role": "user",      "content": [{"text": m.content}] }))
                }
                Role::Assistant => {
                    Some(json!({ "role": "assistant", "content": [{"text": m.content}] }))
                }
            })
            .collect();
...
fn extract_text(resp: &Value) -> Result<String> {
    resp["output"]["message"]["content"]
        ...
        .map(|s| s.to_string())
        .ok_or_else(|| {
            anyhow::anyhow!("Champ 'output.message.content[].text' manquant dans réponse Bedrock")
        })
}
